...
(function(){
  var ref$, lines, unlines, filter, fold, levn, getRaw, filterRegex, replacer, getReplacementFunc, replace, slice$ = [].slice, toString$ = {}.toString;
  ref$ = require('prelude-ls'), lines = ref$.lines, unlines = ref$.unlines, filter = ref$.filter, fold = ref$.fold;
  levn = require('levn');
  ...
  replacer = function(input, node, queryEngine){
    return function(arg$, replacementArg){
      var ref$, selector, filters, that, origResults, e, results, rawPrepend, rawAppend, join, textOperations, filterName, argsStr, args, pre, post, i$, len$, arg, result, n, len, rawResults, res$, outputString;
      ...
      if (origResults.length) {
        ...
        while (filters.length) {
          ...
          if ((filterName === 'prepend' || filterName === 'before' || filterName === 'after' || filterName === 'prepend' || filterName === 'append' || filterName === 'wrap' || filterName === 'nth' || filterName === 'nth-last' || filterName === 'slice' || filterName === 'each' || filterName === 'replace' || filterName === 'substring' || filterName === 'substr') && !args.length) {
  ...
  replace = function(replacement, input, nodes, queryEngine){
    var inputLines, colOffset, lineOffset, lastLine, prevNode, replacementFunc, i$, len$, node, ref$, start, end, startLineNum, endLineNum, numberOfLines, startCol, endCol, replaceLines, startLine, endLine, startContext, endContext, replaceLast, endLen;
    ...
    replacementFunc = getReplacementFunc(replacement, input, queryEngine);
    for (i$ = 0, len$ = nodes.length; i$ < len$; ++i$) {
      ...
      replaceLines = lines(replacementFunc(node));
